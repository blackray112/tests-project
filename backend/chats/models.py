...
class VirtualFriend(Model, table=True):
    ...
    @classmethod
    def get(cls, virtual_friend_id: int) -> Self:
        return super().get(virtual_friend_id)


class Chat(Model, table=True):
    virtual_friend_id: int = Field(foreign_key='virtualfriend.id')
...
class Message(Model, table=True):
    ...
    @classmethod
    def get(cls, message_id: int) -> Self:
